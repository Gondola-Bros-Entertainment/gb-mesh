...
--
-- Layout per vertex (12 floats, 48 bytes):
--
-- @
-- [px, py, pz, nx, ny, nz, u, v, tx, ty, tz, tw]
-- @
--
-- Total length: @12 * vertexCount mesh@.
packInterleaved :: Mesh -> [Float]
packInterleaved mesh = concatMap packVertex (meshVertices mesh)
  where
    packVertex (Vertex (V3 px py pz) (V3 nx ny nz) (V2 u v) (V4 tx ty tz tw)) =
      [px, py, pz, nx, ny, nz, u, v, tx, ty, tz, tw]
...
packPositions mesh = concatMap packPos (meshVertices mesh)
  where
    packPos (Vertex (V3 px py pz) _ _ _) = [px, py, pz]
...
packNormals mesh = concatMap packNrm (meshVertices mesh)
  where
    packNrm (Vertex _ (V3 nx ny nz) _ _) = [nx, ny, nz]
...
packUVs mesh = concatMap packUV (meshVertices mesh)
  where
    packUV (Vertex _ _ (V2 u v) _) = [u, v]
...
packTangents mesh = concatMap packTan (meshVertices mesh)
  where
    packTan (Vertex _ _ _ (V4 tx ty tz tw)) = [tx, ty, tz, tw]
...
--
-- Returns four 'AttribLayout' values for position (0), normal (1),
-- UV (2), and tangent (3), using the standard interleaved stride.
